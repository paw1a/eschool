...
def generate_data(user_count, school_count, course_count, lesson_count, test_count, output_file):
    ...
    # Generate Courses
    sql_str = ''
    sql_str += "insert into public.course (id, name, school_id, level, price, language, status, rating) values "
    courses = []
    statuses = ['draft']
    ...
    for i in range(0, course_count):
        ...
        sql_str += f"('{course.id}', '{course.name}', '{course.school_id}', {course.level}, "\
                   f"{course.price}, '{course.language}', '{course.status}', {course.rating}),\n"
